...
// PaaS per-cloudlet monthly price in Kč (configurable; default 152.5 for the
// first 10 cloudlets, 133.8 from the 11th cloudlet on — tiered volume pricing).
const CLOUDLET_TIER1 = 10
const CLOUDLET_RATE_CZK = parseFloat(process.env.IaaS_CLOUDLET_RATE_CZK) || 152.5
const CLOUDLET_RATE_TIER2_CZK = parseFloat(process.env.IaaS_CLOUDLET_RATE_TIER2_CZK) || 133.8

function cloudletCost(totalCl) {
  if (totalCl <= CLOUDLET_TIER1) return totalCl * CLOUDLET_RATE_CZK
  return CLOUDLET_TIER1 * CLOUDLET_RATE_CZK + (totalCl - CLOUDLET_TIER1) * CLOUDLET_RATE_TIER2_CZK
}
...
function summarize(nodes, cm = DEFAULT_COMMITMENT) {
  ...
  return {
    ...
    cloudletRateTier2CZK: CLOUDLET_RATE_TIER2_CZK,
