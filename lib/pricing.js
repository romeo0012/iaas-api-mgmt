...
// dynamic (placené dle utilizace).
const CLOUDLET_CPU_GHZ = 0.4
const CLOUDLET_RAM_GB = 0.125
const CLOUDLET_BANDS = [16, 32, 64, 128, Infinity]
...
const CLOUDLET_DISK_RATE_CZK = 2.4 // cena / GB / měsíc (0.003286 Kč/h × 730)
const CLOUDLET_PUBLIC_IP_RATE_CZK = 120.01 // cena / IP / měsíc (0.1644 Kč/h × 730)
const CLOUDLET_RESERVED_PCT = 15 // rezervované cloudlety = vždy placené minimum (15 % z celku)
...
// Public IP monthly fee (T-Business kalkulačka: 108 Kč/IP). Billed only when
// the topology contains a firewall (group `opnsense`). Configurable.
...
function summarize(nodes, cm = DEFAULT_COMMITMENT) {
  ...
  totals.totalCZK = totals.cpuCostCZK + totals.ramCostCZK + totals.diskCostCZK + totals.networkingFwCZK + totals.publicIpCZK
