...
// Cloudlet (PaaS/Virtuozzo): 1 cloudlet = 128 MiB RAM (= 0.125 GiB) and 400 MHz
// CPU (= 0.4 GHz). Per-VM cloudlets = max(CPU/0.4, RAM/0.125), rounded up; total =
// sum across all VMs. Disks are ignored for the cloudlet count. Virtuozzo volume
// pricing bands (Kč/cloudlet/month = hourly × 730 h): reserved (vždy placené) and
// dynamic (placené dle utilizace). All values configurable via .env.
const CLOUDLET_CPU_GHZ = 0.4
const CLOUDLET_RAM_GB = 0.125

function parseRateList(v, fallback) {
  if (!v) return fallback.slice()
  const arr = String(v).split(/[,\s;]+/).map(Number).filter(n => Number.isFinite(n))
  return arr.length ? arr : fallback.slice()
}
const CLOUDLET_RESERVED_RATES = parseRateList(process.env.IaaS_PAAS_RESERVED_RATES, [98.84, 93.88, 88.91, 84.02, 79.06])
const CLOUDLET_DYNAMIC_RATES = parseRateList(process.env.IaaS_PAAS_DYNAMIC_RATES, [148.26, 144.54, 140.82, 137.09, 133.44])
const CLOUDLET_BANDS = (() => {
  const v = String(process.env.IaaS_PAAS_BANDS || '').split(/[,\s;]+/).map(Number).filter(Number.isFinite)
  return v.length && v[v.length - 1] !== Infinity ? v.concat([Infinity]) : (v.length ? v : [16, 32, 64, 128, Infinity])
})()
// PaaS disk sazba: default je IaaS tier "Standard" dle závazku (0→1.95 / 12→1.35 /
// 24→1.28 / 36→1.20 Kč/GB/měs). Explicitní IaaS_PAAS_DISK_RATE_CZK ji přebije plochou sazbou.
const PAAS_DISK_RATE_OVERRIDE = (() => {
  const v = parseFloat(process.env.IaaS_PAAS_DISK_RATE_CZK)
  return Number.isFinite(v) && v > 0 ? v : null
})()
const CLOUDLET_PUBLIC_IP_RATE_CZK = parseFloat(process.env.IaaS_PAAS_PUBLIC_IP_RATE_CZK) || 120.01 // cena / IP / měsíc
const CLOUDLET_RESERVED_PCT = parseFloat(process.env.IaaS_PAAS_RESERVATION_PCT) || 15 // rezervované = vždy placené minimum (% z celku)
const DEFAULT_PAAS_UTILIZATION = (() => {
  const v = parseFloat(process.env.IaaS_PAAS_UTILIZATION)
  return Number.isFinite(v) ? Math.min(Math.max(v / 100, 0.1), 1) : 0.4
})()

function cloudletBandedCost(count, rateArr) {
  let prev = 0
  let cost = 0
  for (let i = 0; i < CLOUDLET_BANDS.length; i++) {
    if (count <= prev) break
    const inBand = Math.min(count, CLOUDLET_BANDS[i]) - prev
    if (inBand > 0) cost += inBand * rateArr[i]
    prev = CLOUDLET_BANDS[i]
  }
  return cost
}

function cloudletSplit(totalCl, util) {
  const reserved = Math.ceil(totalCl * CLOUDLET_RESERVED_PCT / 100)
  const dynamic = Math.max(0, Math.ceil(totalCl * util) - reserved)
  return { reserved, dynamic }
}

function cloudletCost(totalCl, util) {
  const u = util != null ? util : DEFAULT_PAAS_UTILIZATION
  const { reserved, dynamic } = cloudletSplit(totalCl, u)
  return cloudletBandedCost(reserved, CLOUDLET_RESERVED_RATES) + cloudletBandedCost(dynamic, CLOUDLET_DYNAMIC_RATES)
}
const PUBLIC_IP_RATE_CZK = parseFloat(process.env.IaaS_PUBLIC_IP_RATE_CZK) || 108
// Remote backup: monthly fee per GB of total disk (Kč/GB/měs). Configurable.
const REMOTE_BACKUP_RATE_CZK = parseFloat(process.env.IaaS_REMOTE_BACKUP_RATE_CZK) || 0.68
// Remote backup counts twice the total disk (capacity × 2).
...
// .env overrides apply to whatever commitment is active; empty string = table rates.
const ENV = {
...
function rateCpuGHz(cm = DEFAULT_COMMITMENT) {
  ...
  return (Number.isFinite(ENV.cpu) && cm === DEFAULT_COMMITMENT) ? ENV.cpu : table
}
function rateRamGB(cm = DEFAULT_COMMITMENT) {
  const table = RATES.ram[cm]
  return (Number.isFinite(ENV.ram) && cm === DEFAULT_COMMITMENT) ? ENV.ram : table
}
function rateDiskGB(cm = DEFAULT_COMMITMENT) {
  const table = DISK_TIERS.superfast.rates[cm]
  return (Number.isFinite(ENV.disk) && cm === DEFAULT_COMMITMENT) ? ENV.disk : table
}
...
function nodeCost(node, cm = DEFAULT_COMMITMENT) {
  ...
  // Disk is billed only at the VM's own performance tier rate (no synthetic
  // "Basic base" on top).
...
// Nodes flagged `excludeIaaS: true` are still listed (iaasExcluded: true, zeroed
// cost fields, so the client can render them greyed out) but contribute nothing
// to the pooled totals / remote backup / disk tiers / Public IP.
function summarize(nodes, cm = DEFAULT_COMMITMENT, remoteBackupGB = null) {
  const costed = []
  const perNode = nodes.map(n => {
    const c = nodeCost(n, cm)
    if (n.excludeIaaS) {
      return { ...c, iaasExcluded: true, cpuCostCZK: 0, ramCostCZK: 0, diskCostCZK: 0, totalCZK: 0, totalFormatted: '—' }
    }
    costed.push(c)
    return c
  })
  ...
  // Remote backup capacity is editable in the UI. Explicit value (incl. 0 =
  // disabled) is used as-is; when omitted/null it defaults to 2× total disk.
  ...
  const totals = {
    cloudlets: costed.reduce((s, n) => s + n.cloudlets, 0),
  ...
  // Disk total broken down by the tiers actually used in the topology
  // (Super Fast / Fast / Standard / Basic). No synthetic rows — a tier row
  // appears only when at least one VM uses that tier.
  const TIER_ORDER = ['superfast', 'fast', 'standard', 'basic']
  const tierBreakdown = TIER_ORDER.map(key => {
    const t = DISK_TIERS[key]
    const tierNodes = costed.filter(n => n.diskTier === key)
  ...
  const diskByTier = tierBreakdown
  ...
  return {
    ...
    cloudletReservedRates: CLOUDLET_RESERVED_RATES,
    cloudletDynamicRates: CLOUDLET_DYNAMIC_RATES,
    cloudletDiskRateCZK: DISK_TIERS.standard.rates[DEFAULT_COMMITMENT],
    cloudletPublicIpRateCZK: CLOUDLET_PUBLIC_IP_RATE_CZK,
    ...
    remoteBackupDefaultGB: pooled.diskGB * REMOTE_BACKUP_MULTIPLIER,
    remoteBackupAuto: remoteBackupGB == null,
    ...
    cloudletCostCZK: cloudletCost(totals.cloudlets),
...
function paasConfig() {
  return {
    ...
    reservationPct: CLOUDLET_RESERVED_PCT,
    diskRateCzk: PAAS_DISK_RATE_OVERRIDE,
    standardDiskRates: { ...DISK_TIERS.standard.rates },
    publicIpRateCzk: CLOUDLET_PUBLIC_IP_RATE_CZK,
    utilizationPct: Math.round(DEFAULT_PAAS_UTILIZATION * 100),
  }
}
