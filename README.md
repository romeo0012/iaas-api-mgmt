...
- **Defaultní topologie** (build-in): `Sec-01` (OPNsense, Fast, 4+4+50) + `App-01` (Standard, 8+4+100) + `Db-01` (Fast, 4+4+300). Pokud existuje `default.topo.json` (formát stejný jako `.topo.json` export UI), načte se **při startu serveru** místo ní.
  - Dodávané `default.topo.json`: `envName = ""`, skupinové labely `Aplikace` / `Databáze` / `Bezpečnost`, `Sec-01` label `Firewall/LoadBalancing`. Skupina firewallu je `opnsense`; u ručně nahraných `.topo.json` s `security` ji `compute()` v `lib/architecture.js` **aliasuje na `opnsense`**, takže uzel zůstává v topologii (renderuje se jako firewall s veřejnou IP na NIC).
